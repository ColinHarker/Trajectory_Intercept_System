...
[[nodiscard]] int calculateTrajectory(std::array< T, SIZE >& p) {
    ...
    std::array<int, constants::kNumPredictedFrames> f_position;


    for (size_t i = 0; i < constants::kNumCalcFrames - 1; i++) {
    ...
    for (size_t i = 1; i < constants::kNumPredictedFrames; i++) {
        if (c_s_velocity != 0) {
